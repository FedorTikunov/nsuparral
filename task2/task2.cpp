...
int main() {
	...
	int num_blocks = 18, block_size = ((GRID_SIZE - 2) / num_blocks) * (GRID_SIZE - 2);
	#pragma acc data copy (olda[0:(GRID_SIZE * GRID_SIZE)], newa[0:(GRID_SIZE * GRID_SIZE)]) 
	{
		...
		while (iter_count < ITER && error > ACC) {
			...
			for (int block = 0; block < num_blocks; block++) {
				...
				#pragma acc parallel loop independent reduction(max:error) collapse(2) gang num_gangs(256) vector vector_length(256) present(olda[0:GRID_SIZE * GRID_SIZE], newa[0:GRID_SIZE * GRID_SIZE]) async(block)
