...
int main() {
	...
	int num_blocks = 8, block_size = ((GRID_SIZE - 2) / num_blocks) * (GRID_SIZE - 2);
...
#pragma acc parallel loop gang num_gangs(256) vector vector_length(256) present(olda[0:(GRID_SIZE * GRID_SIZE)], newa[0:(GRID_SIZE * GRID_SIZE)]) 
		...
		while (iter_count < ITER && error > ACC) {
			...
			for (int block = 0; block < num_blocks; block++) {
				int ystart = block * (GRID_SIZE - 2 / num_blocks) + 1,
					yend = ystart + (GRID_SIZE - 2 / num_blocks);
				#pragma acc parallel loop independent reduction(max:error) collapse(2) gang num_gangs(256) vector vector_length(256) present(olda[(block*block_size + 1):block_size], newa[(block*block_size + 1):block_size]) async(block)
				...
				#pragma acc update self(olda[(block*block_size + 1):block_size], newa[(block*block_size + 1):block_size])  async(block)
			}
			# pragma acc wait
			//printf("3: %lf\n", error);
			iter_count++;
